  ...
  try {
    ...
    if (err.code === 11000) {
      ...
      return res
        ...
        .json({
          message:
            "Username already exists. Please choose another or load game.",
        });
    }
    console.error(err);
...
app.post("/players/:id/collect/:pieceId", async (req, res) => {
  ...
  try {
    ...
    if (player.puzzlePieces.includes(pieceId)) {
      return res
        .status(400)
        .json({ message: "Player already has this puzzle piece." });
    }
    ...
    res.json({
      message: `Puzzle piece "${puzzlePiece.name}" collected!`,
      player: player,
    });
  } catch (err) {
    console.error(err);
...
app.post("/game/new", async (req, res) => {
  try {
    ...
    if (sprite && typeof sprite !== "string") {
      return res
        .status(400)
        .json({ message: "Sprite must be a string (URL or filename)." });
    }
